...
export async function createTask(req: Request, res: Response) {
  const { title, description, dueDate, priority, status, recurring, recurringType, tags } = req.body;
...
export async function updateTask(req: Request, res: Response) {
  ...
  const task = await Task.findOneAndUpdate({ _id: req.params.id, userId: req.user!.id }, update, {
    new: true,
  });
